...
############### partial correlation with water storage
...
gridcorts <- function(rasterstack, method, type=c("corel","pval","both")){
  ...
  empt <- matrix(nrow=ncell, ncol=4)  ### ncol here depends on the number of variables in the stack. In my
  print("Initiating loop operation")    # case its 4 variables (ET, rain, radiation, AED)
  
  ### I didn't edit this part because I needed both correlation and pvalue.
  ### So this part is for correlation and not partial correlation because the stacked layers 
  ### are divided in 2
  ...
  ###### i didn't edit this part either because of the same reason. This part is only for pvalue.
  ...
  ### So this is the part I edited becasue the type=both is my interest. I divided the stacked layers into 
  ### 4 instead of 2 (as in above) in the matrix because my rasterstack contains annual data of 
  ### four variables
    ...
    if (type == "both"){
      for (i in 1:ncell){
        setTxtProgressBar(pb,i)
        if (all(is.na(mtrx[i,1:(layers/4)])) | all(is.na(mtrx[i,((layers/4)+1):(2*layers/4)])) | all(is.na(mtrx[i,((2*layers/4)+1):(3*layers/4)])) | all(is.na(mtrx[i,((3*layers/4)+1):layers]))){ 
          empt[i,] <- NA 
        } else 
          if (sum(!is.na(mtrx[i,1:(layers/2)]/mtrx[i,((layers/2)+1):layers])) < 4 ){
            empt[i,] <- NA 
          } else {
            empt[i,1] <- as.numeric(pcor.test(mtrx[i,1:(layers/4)], mtrx[i,((layers/4)+1):(2*layers/4)], mtrx[i,((2*layers/4)+1):layers], method=method)$estimate) 
            empt[i,2] <- as.numeric(pcor.test(mtrx[i,1:(layers/4)], mtrx[i,((layers/4)+1):(2*layers/4)], mtrx[i,((2*layers/4)+1):layers], method=method)$p.value)
          }
      }
...
###########correlation
##note the order of arrangements of stacked will determine the partial correlation results. On first line the pc is between WC and PR while holding mrro and et constant
ENS_H_qdm_WC_pr_monthly<-gridcorts(stack(ENS_H_qdm_wp_WC_monthly,ENS_H_qdm_wp_pr_monthly,ENS_H_qdm_wp_mrro_monthly,ENS_H_qdm_wp_et_monthly),method = "pearson", type="both")
ENS_H_qdm_WC_mrro_monthly<-gridcorts(stack(ENS_H_qdm_wp_WC_monthly,ENS_H_qdm_wp_mrro_monthly,ENS_H_qdm_wp_pr_monthly,ENS_H_qdm_wp_et_monthly),method = "pearson", type="both")    
ENS_H_qdm_WC_et_monthly<-gridcorts(stack(ENS_H_qdm_wp_WC_monthly,ENS_H_qdm_wp_et_monthly,ENS_H_qdm_wp_mrro_monthly,ENS_H_qdm_wp_pr_monthly),method = "pearson", type="both")

ENS_H_mbcn_WC_pr_monthly<-gridcorts(stack(ENS_H_mbcn_wp_WC_monthly,ENS_H_mbcn_wp_pr_monthly,ENS_H_mbcn_wp_et_monthly,ENS_H_mbcn_wp_mrro_monthly),method = "pearson", type="both")
ENS_H_mbcn_WC_mrro_monthly<-gridcorts(stack(ENS_H_mbcn_wp_WC_monthly,ENS_H_mbcn_wp_mrro_monthly,ENS_H_mbcn_wp_pr_monthly,ENS_H_mbcn_wp_et_monthly),method = "pearson", type="both")    
ENS_H_mbcn_WC_et_monthly<-gridcorts(stack(ENS_H_mbcn_wp_WC_monthly,ENS_H_mbcn_wp_et_monthly,ENS_H_mbcn_wp_mrro_monthly,ENS_H_mbcn_wp_pr_monthly),method = "pearson", type="both")

ENS_H_raw_WC_pr_monthly<-gridcorts(stack(ENS_H_raw_wp_WC_monthly,ENS_H_raw_wp_pr_monthly,ENS_H_raw_wp_et_monthly,ENS_H_raw_wp_mrro_monthly),method = "pearson", type="both")
ENS_H_raw_WC_mrro_monthly<-gridcorts(stack(ENS_H_raw_wp_WC_monthly,ENS_H_raw_wp_mrro_monthly,ENS_H_raw_wp_pr_monthly,ENS_H_raw_wp_et_monthly),method = "pearson", type="both")    
ENS_H_raw_WC_et_monthly<-gridcorts(stack(ENS_H_raw_wp_WC_monthly,ENS_H_raw_wp_et_monthly,ENS_H_raw_wp_mrro_monthly,ENS_H_raw_wp_pr_monthly),method = "pearson", type="both")

Ref_H_mbcn_WC_pr_monthly<-gridcorts(stack(Ref_H_mbcn_wp_WC_monthly,Ref_H_mbcn_wp_pr_monthly,Ref_H_mbcn_wp_et_monthly,Ref_H_mbcn_wp_mrro_monthly),method = "pearson", type="both")
Ref_H_mbcn_WC_mrro_monthly<-gridcorts(stack(Ref_H_mbcn_wp_WC_monthly,Ref_H_mbcn_wp_mrro_monthly,Ref_H_mbcn_wp_pr_monthly,Ref_H_mbcn_wp_et_monthly),method = "pearson", type="both")    
Ref_H_mbcn_WC_et_monthly<-gridcorts(stack(Ref_H_mbcn_wp_WC_monthly,Ref_H_mbcn_wp_et_monthly,Ref_H_mbcn_wp_mrro_monthly,Ref_H_mbcn_wp_pr_monthly),method = "pearson", type="both")
############significant
...
myColorkey<- list(at=brk,space="right", title = list("Partial Correlation ",cex = 0.8,fontface = 7,col = 'black'),
...
cols<-colorRampPalette(c("navyblue","blue","white","pink","red"))
...
A<-levelplot(ENS_H_qdm_WC_pr_monthly$Correlation,colorkey=F,#layout=c(3,3), # create a 4x4 layout for the data
             ...
             par.settings=list(panel.background=list(col="grey90"),axis.line=list(lwd=0.8,col="blue"), #background colour, axis line 
             ...
             ylab=list('', rot=90, fontface='bold'))+layer(sp.polygons(basin, lwd=2,col = "black"))+
  layer(sp.polygons(wp, lwd=1,col = "black"))+layer(sp.polygons(b16, lwd=2,col = "black"))+
